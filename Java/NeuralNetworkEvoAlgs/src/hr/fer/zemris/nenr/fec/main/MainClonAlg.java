...
public class MainClonAlg {
	
	private static Antibody getOptimalWeights(Dataset dataset, NeuralNetwork neuralNetwork) {
		DefinedEvaluator evaluator = new DefinedEvaluator(neuralNetwork, dataset);
		int chromosomeLength = neuralNetwork.getWeightsLength();
		ClonAlg clonAlg = new ClonAlg(evaluator, chromosomeLength);
		
		Antibody bestAntibody = clonAlg.train();
		
		System.out.println(evaluator.calcError(bestAntibody.get()));
		return bestAntibody;
	}
	...
	public static void main(String[] args) throws IOException {
		...
		Antibody bestAntibody = getOptimalWeights(ds, neuralNetwork);
